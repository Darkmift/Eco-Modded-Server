...
namespace Eco.Mods.Organisms
{
    ...
    public class AmphibiousBrain : LandAnimalBrain 
    {
        ...
        {
            LandTree = BT.If("Land Tree", x => (!World.World.GetBlock((Vector3i)x.Position).IsWater(), "on land"),
                    BT.Selector("Amphibious On Land",
                        ...
                        BT.If("Return to Water", x => (x.Position.Y < WorldLayerManager.Obj.ClimateSim.State.SeaLevel + MaxHeightAboveSeaLevel, $"above sea-level {(x.Position.Y + MaxHeightAboveSeaLevel).ToString("0.#")} not more than {WorldLayerManager.Obj.ClimateSim.State.SeaLevel}"),
                            MovementBehaviors.MoveToWater),
                        BT.If("End Wander", x => x.CheckAndConsumeDoNotContinueToWanderFlag() ? (true, "decided to stop") : (false, "decided to stop"),
            ...
            AmphibiousTreeRoot         = BT.Selector("Amphibious", 
                BT.If("Animal On Land", x => (!World.World.GetBlock((Vector3i)x.Position).IsWater(), "on land"),
                    BT.Selector("Landing",
                        MovementBehaviors.Flee,
                        FindAndEatPlants,
                        LandTree)),
                BT.If("Animal In Water", x => (World.World.GetBlock((Vector3i)x.Position).IsWater(), "in water"),
            ...
            PredatorAmphibiousTreeRoot = BT.Selector("Amphibious Predator",
                BT.If("Predator On Land", x => (!World.World.GetBlock((Vector3i)x.Position).IsWater(), "on land"),
                    BT.Selector("Landing",
                        MovementBehaviors.Flee,
                ...
                BT.If("Predator In Water", x => (World.World.GetBlock((Vector3i)x.Position).IsWater(), "in water"),
                    BT.Selector("Swimming",
                        MovementBehaviors.SwimFlee,
                        FloatingTree,
                        DivingTree)
